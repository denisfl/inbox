...
var document_editor_controller_default = class extends Controller {
  ...
  setupDragAndDrop() {
    ...
    this.blocksContainerTarget.addEventListener("dragstart", this.handleDragStart.bind(this));
    this.blocksContainerTarget.addEventListener("dragend", this.handleDragEnd.bind(this));
    this.blocksContainerTarget.addEventListener("dragover", this.handleDragOver.bind(this));
    this.blocksContainerTarget.addEventListener("drop", this.handleDrop.bind(this));
    this.blocksContainerTarget.addEventListener("dragleave", this.handleDragLeave.bind(this));
  }
  ...
  handleDrop(event) {
    ...
    const newPosition = this.calculateNewPosition(targetBlock, dropBefore);
    ...
    if (blockId && blockId !== "null") {
      this.reorderBlock(blockId, newPosition);
    }
  }
  calculateNewPosition(targetBlock, dropBefore) {
    const allBlocks = Array.from(this.blocksContainerTarget.querySelectorAll(".block-wrapper"));
  ...
  async reorderBlock(blockId, newPosition) {
    console.log("Reordering block via API:", { blockId, newPosition });
    console.log("Auth token:", this.getAuthToken());
    try {
      const url = `/api/documents/${this.documentIdValue}/blocks/${blockId}/reorder?new_position=${newPosition}`;
      ...
      if (!response.ok) {
        ...
        throw new Error("Failed to reorder block");
      }
      const data = await response.json();
      ...
      this.showSaveIndicator("error");
      alert("Failed to reorder block. Please refresh the page.");
    }
  }
  updateBlockPositions(blocks) {
    console.log("Updating block positions:", blocks);
    blocks.forEach(({ id, position }) => {
      const wrapper = this.blocksContainerTarget.querySelector(`[data-block-id="${id}"]`);
      if (wrapper) {
...
@hotwired/turbo/dist/turbo.es2017-esm.js:
   ...
   *)
*/
//# sourceMappingURL=/assets/application-ce27c0af.js.map
