...
#### Scenario: Failed external call
- **WHEN** an HTTP call to an external service fails
- **THEN** the system SHALL log the failure at `error` level with tags `[service_name]`, method, URL, error class, error message, and attempt number

### Requirement: SolidQueue job retry standardization
All SolidQueue jobs that interact with external services SHALL have explicit `retry_on` and `discard_on` configuration.

#### Scenario: Transient job failure
- **WHEN** a background job fails due to a transient error (network, timeout)
- **THEN** the job SHALL retry with `wait: :polynomially_longer` up to the configured attempt limit

#### Scenario: Permanent job failure
- **WHEN** a background job fails due to a permanent error (invalid input, unsupported format)
- **THEN** the job SHALL be discarded with the error logged
