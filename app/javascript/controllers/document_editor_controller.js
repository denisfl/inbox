...
export default class extends Controller {
  ...
  autoFocusFirstBlock() {
    ...
    const firstEditable = this.blocksContainerTarget.querySelector(
  ...
  handleKeydown(event) {
    ...
    if ((event.metaKey || event.ctrlKey) && event.shiftKey) {
      ...
      if (shortcuts[event.key]) {
        ...
        this.createBlockOnServer(newBlock).then(() => {
          ...
          if (lastBlock) {
            const contentEditable = lastBlock.querySelector(
              "[contenteditable=true]",
            );
            if (contentEditable) {
    ...
    if (
      ...
      if (blockWrapper && activeElement.hasAttribute("contenteditable")) {
        ...
        if (blockType === "text") {
          const handled = this.handleSmartEnter(
            event,
            blockWrapper,
            activeElement,
          );
  ...
  handleSmartEnter(event, blockWrapper, activeElement) {
    ...
    const currentLine = fullText
      .substring(currentLineStart, cursorPosition)
      .trim();
  ...
  detectMarkdownPattern(line) {
    const patterns = [
      ...
      {
        regex: /^(#{1,3})\s+(.*)$/,
        handler: (m) => ({ type: "heading", level: m[1].length, text: m[2] }),
      },
      ...
      {
        regex: /^-\s*\[\s*\](?:\s+(.*))?$/,
        handler: (m) => ({ type: "todo", text: m[1] || "", checked: false }),
      },
      // Slash todo
      {
        regex: /^\/todo(?:\s+(.*))?$/,
  ...
  handleDoubleEnter(blockWrapper, activeElement, cursorPosition) {
    ...
    console.log("Double Enter split:", {
  ...
  createNewBlockAfter(afterPosition, blockType, content, additionalText = "") {
    ...
    return this.createBlockOnServer(newBlock)
      .then((response) => {
        if (!response) {
        ...
        const currentBlock = this.blocksContainerTarget.querySelector(
          `[data-position="${afterPosition}"]`,
        );
        if (currentBlock) {
          const newElement = this.createBlockElement({
            id: response.id,
            ...newBlock,
          });
  ...
  handleTodoEnter(blockWrapper, activeElement) {
    ...
    const blockId = blockWrapper.dataset.blockId;
    const checked =
      blockWrapper.querySelector("input[type=checkbox]")?.checked || false;
    ...
    this.createBlockOnServer(newBlock).then(() => {
      ...
      setTimeout(() => {
        const todoSpan = newBlockElement.querySelector(
        ...
        return {
          text:
            blockContent.querySelector(
              "textarea[data-markdown-editor-target='textarea']",
            )?.value ||
  ...
  setupPasteHandler() {
    ...
    this.blocksContainerTarget.addEventListener("paste", (event) => {
      ...
      if (pastedText.includes("\n")) {
        ...
        if (
          blockWrapper &&
          blockWrapper.dataset.blockType === "text" &&
  ...
  async createBlocksSequentially(blocks, afterPosition) {
    for (let i = 0; i < blocks.length; i++) {
      ...
      try {
        await this.createNewBlockAfter(
          afterPosition + i,
          block.type,
          block,
          "",
    ...
    if (newType) {
      ...
      if (!blockId || blockId === "null") {
        ...
        this.updateBlockOnServer(blockId, {
          ...
          // Focus the new block
          const contentEditable = blockWrapper.querySelector(
            "[contenteditable=true]",
          );
          if (contentEditable) {
            contentEditable.focus();
  ...
  showSlashMenu(element, commands, slashIndex) {
    ...
    commands.forEach((cmd, index) => {
      ...
      item.style.borderBottom =
        index < commands.length - 1 ? "1px solid #eee" : "none";
